...
/////////////////////////////////\
int IsPowerOfTwo(int x)//will return 1 if power of two else 0\
\{\
 return ((x != 0) && ((x & (x - 1)) == 0));\
\}\
int FNLPOT(int x)//find next left (previous) power of two\
\{\
  x = x | (x >> 1);\
  x = x | (x >> 2);\
  x = x | (x >> 4);\
  ...
  return (x - (x >> 1));\
\}\
int GetTarget( int p, int s)//get target according to step and id\
\{\
  int NLPOT;/// variable to store previous power of two if step is not power of two\
  int target;\
        int diff;\
  int flag = p /pow(2,s-1);// to group the process on the basis of steps\
  if (flag%2==0)\
  \{\
    if (IsPowerOfTwo(s))//checks is step power of two?\
      \{\
        target = p + pow(2, s - 1);\
      \}\
    else\
      \{\
        NLPOT = FNLPOT(s);// if step is not power of two then find the previous power of two that is nearest to the step number\
        diff=s-NLPOT; //calculates the diffrence between current step and the previous power of two to that step\
        if(diff>1)\
          target = p + pow(2, NLPOT+diff-1);\
        else\
          target=p+pow(2, NLPOT);\
      \}\
  \}\
  else\
    \{\
...
int org_arr[1024];\
int target_arr[1024];\
void RMA_Allgather(int *origin_addr, int origin_count, MPI_Datatype origin_datatype, int *recvbuf,  int recvcount, MPI_Datatype recvtype, MPI_Comm comm)\
\{\
   int i=1;\
\
   int target,disp;\
...
\
   if(first_done == 0) \{\
     ...
     recv_global_buf = (int*)hoc_Emalloc(origin_count*num_procs*sizeof(int)); hoc_malchk();;\
     MPI_Win_create(recv_global_buf, origin_count*num_procs, sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &win);\
    for(i=1;i<=log;i=i++) \{\
       org_arr[i] = (id-(id%(int) pow(2,i-1)));\
       //    MPI_Win_fence(0, win);\
       target_arr[i] = GetTarget(id, i);\
     \}\
     first_done = 1;\
   \}\
\
...
\
   //Window creation\
   // zeki MPI_Win_create(recvbuf, origin_count*num_procs, sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &win);\
\
//   MPI_Win_fence(0, win);\
\
   recv_global_buf[id] = origin_addr[0];\
   int step_jump=0;\
  //    MPI_Win_fence(0, win);\
\
   for(i=1;i<=log;i=i++) \{\
     //org = (id-(id%(int) pow(2,i-1)));\
//    MPI_Win_fence(0, win);\
    //target = GetTarget(id, i);\
     MPI_Win_fence(0, win);\
     int j=pow(2,i-1);\
     disp=org;\
     MPI_Put(&recv_global_buf[org_arr[i]],j,MPI_INT,target_arr[i],disp,j,MPI_INT,win);\
//     MPI_Win_fence(0, win);\
\
   \}\
   MPI_Win_fence(0, win);\
   // zeki MPI_Win_free(&win);\
   for(i=0;i<origin_count*num_procs;i=i++)\
     recvbuf[i] = recv_global_buf[i];\
\
\}\
\
\
...
\
 if(getenv("RMA"))\
         RMA_Allgather(&nout_, 1, MPI_INT, nin_, 1, MPI_INT, nrnmpi_comm);\
  else\
        MPI_Allgather(&nout_, 1, MPI_INT, nin_, 1, MPI_INT, nrnmpi_comm);\
\
/////////////\
t2 = MPI_Wtime();\
allgather_time += t2-t1;\
////////////\
        n = nin_[0];\
        for (i=1; i < np; ++i) \{\
                displs[i] = n;\
